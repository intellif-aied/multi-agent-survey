...
本报告基于四个项目的本地源码，比较一个整体任务如何被拆给多个 Agent，以及这些分工如何落实为可运行、可通信、可恢复的 Agent 进程或会话。这里研究的不是单个 Agent 如何推理，而是多 Agent 系统的工程边界：调度的对象是什么、谁决定把工作交给谁、谁保存最终状态、任务如何进入执行进程、结果如何返回、故障后从哪里恢复。
